...
def test_click_bonus_program(driver):
    ...
    main_page.assert_that_bonus_program_banner_is_exist()
...
def test_assert_show_more_button(driver):
    ...
    assert count_popular_els_old > 0, "No popular elements found before clicking 'Show More'."
    driver.execute_script("window.scrollBy(0, 1000);")
    time.sleep(1)
    main_page.click_show_more_popular_elements()
    count_popular_els_new = main_page.get_count_popular_elements()
    assert count_popular_els_new > count_popular_els_old
...
def test_check_popular_filter_cheap(driver):
    ...
    main_page.assert_first_product_of_populars_is_filtered()
