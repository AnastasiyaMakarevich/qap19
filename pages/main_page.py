...
class MainPage(BasePage, MainPageLocators):
    ...
    @allure.step('Assert that bonus program banner is exist')
    def assert_that_bonus_program_banner_is_exist(self):
        self.assertions.assert_that_element_is_visible(
    ...
    def click_search_headphones(self):
        ...
        if search_field.get_attribute('value') == '':
            search_field.send_keys('наушники')
    ...
    @allure.step('Assert first product of populars is filtered')
    def assert_first_product_of_populars_is_filtered(self):
        time.sleep(1)
    ...
    def click_write_to_us(self):
        self.driver.execute_script("arguments[0].scrollIntoView();", self.get_element(self.WRITE_TO_US))
    ...
    def assert_that_email_is_not_specified_in_subscription_form(self):
        email_input = self.driver.find_element(*self.BUTTON_INPUT_EMAIL)
        assert email_input.get_attribute('value') == "", "Email input should be empty"

    @allure.step('Assert that email is not specified in subscription form')
    def assert_that_email_is_not_specified_in_subscription_form(self):
        error_icon_element = self.get_element(self.INPUT_ERROR_MESSAGE)
        assert error_icon_element.is_displayed(), 'Error icon is not displayed'
